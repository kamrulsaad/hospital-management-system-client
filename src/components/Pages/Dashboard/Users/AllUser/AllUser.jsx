...
const AllUser = () => {
  ...
  useEffect(() => {
    ...
    fetch(
      `https://server.thelabaidhospital.com/api/v1/user/all-user?page=${state.pageNumber}&limit=${state.size}&key=${state.key}&value=${state.value}`,
      {
        headers: {
  ...
  return (
    <div className="p-10">
      <h1 className="text-3xl font-bold ">Users : {state.count}</h1>
          ...
          <tbody>
            {state.doctors.map((doctor, i) => (
              <tr key={doctor._id}>
                ...
                <td className="text-center">{doctor?.serialId || "Super-Admin"}</td>
                ...
                <td className="text-center capitalize">{doctor?.role}</td>
