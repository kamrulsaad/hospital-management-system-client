...
const AllInvoice = () => {
  ...
  const handleExport = async () => {
    try {
      const response = await fetch(
        "https://server.thelabaidhospital.com/api/v1/invoice/monthly-invoices",
        {
          headers: {
  ...
  useEffect(() => {
    ...
    fetch(
      `https://server.thelabaidhospital.com/api/v1/invoice/all-invoices?page=${state.pageNumber}&limit=${state.size}&key=${state.key}&value=${state.value}`,
      {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("LoginToken")}`,
        },
